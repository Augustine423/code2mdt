...
export const fetchVesselsByCompanyId = createAsyncThunk(
  ...
  async ({ companyId, page = 0 }, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        `${VITE_API_URL}/aioceaneye/vessels/company/${companyId}?page=${page}`
      );
...
const vesselSlice = createSlice({
  name: "vessels",
  initialState: {
    vessels: [],  // Initialize as an empty array
    status: "idle",
    error: null,
    loading: false,
    totalPages: 0,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchVessels.fulfilled, (state, action) => {
        state.loading = false;
        state.vessels = action.payload.contents || [];  // Ensure vessels is an array
        state.totalPages = action.payload.totalPages || 0;
        state.currentPage = action.payload.currentPage || 1;
      })
      .addCase(fetchVessels.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || "An error occurred while fetching vessels.";
      })
      ...
      .addCase(fetchVesselsByCompanyId.fulfilled, (state, action) => {
        state.loading = false;
        state.vessels = action.payload.contents || [];
        state.totalPages = action.payload.totalPages || 0;
        state.currentPage = action.payload.currentPage || 1;
      })
