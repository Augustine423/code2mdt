...
//  const VITE_API_URL ="http://52.79.57.3:8080"
...
export const fetchDroneInfos = createAsyncThunk(
  "drones/fetchDroneInfos",
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get("http://52.79.57.3:3003/droneInfo");
      return response.data;  // Directly return response.data without extra await
    } catch (error) {
      return rejectWithValue(error.response?.data || "Failed to fetch dronesS");
    }
  }
);
...
const droneSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchDroneInfos.pending, (state) => {
        state.loading = true;
      })
      .addCase(fetchDroneInfos.fulfilled, (state, action) => {
        state.loading = false;
        state.drones = action.payload;
      })
      .addCase(fetchDroneInfos.rejected, (state, action) => {
        state.loading = false;
